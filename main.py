...
file_path_p = "pronouns.csv"
def load_pronouns_data():
    data = []
    with open(file_path_p, encoding="utf-8-sig") as f:
        reader = csv.DictReader(f)
        for row in reader:
            cleaned_row = {k.strip(): v.strip() for k, v in row.items()}
            data.append(cleaned_row)
    return data
pronouns_data = load_pronouns_data()
class ConnectedPronouns(MDScreen):
    def on_pre_enter(self, *args):
        global pronouns_data
        if not any('arabic_fixed' in entry for entry in pronouns_data):
            for entry in pronouns_data:
                if isinstance(entry['verb'], str):
                    entry['verb'] = fix_arabic_text(entry['verb'])
                entry['arabic_fixed'] = True
        ...
        self.ids.pronouns_label.text = ""  # إفراغ النتائج السابقة إن وجدت
        self.ids.pronouns_input.str = ""
        self.ids.pronouns_input.focus = True

    def on_search_pronouns(self):
        search_term = self.ids.pronouns_input.text.strip()
        if not search_term:
            return
        results = [entry for entry in pronouns_data if entry['verb'] == search_term]
        if results:
            result_text = ""
            ...
            self.ids.pronouns_label.text = result_text
            self.ids.pronouns_input.text = ""
            self.ids.pronouns_input.str = ""
        else:
            create_popup_pronoun()
            self.ids.pronouns_label.text = ""
        self.ids.pronouns_input.focus = True
...
class DictionaryScreen(MDScreen):
    def on_pre_enter(self, *args):
        ...
        if not any('arabic_fixed' in entry for entry in dictionary_data):
            for entry in dictionary_data:
                if isinstance(entry['arabic'], str):
                    entry['arabic'] = fix_arabic_text(entry['arabic'])
                entry['arabic_fixed'] = True
    ...
    def on_search(self):
        ...
        if not search_term:
            return
        results = [entry for entry in dictionary_data if entry['arabic'] == search_term]
        if results:
            result_text = ""
            for row in results:
                result_text += fix_arabic_text(
                    f"العربية: {(fix_arabic_text(row['arabic']))}\n"
                    f"الشاوية: {row['amazigh']}\n"
                    f"النطق: {row['latin']}\n"
                    f"الجمع: {row['plural']}\n"
                    f"النطق للجمع: {row['latinplural']}\n"
                    f"المنطقة: {row['origin']}\n\n"
                )
            ...
            if favorite_screen.word_exists_in_favorites(row['arabic']):
                ...
                self.ids.favorite_button.icon = "heart-outline"
            self.ids.favorite_button.on_release = lambda: self.add_word_to_favorites(
                row['arabic'], fix_arabic_text(row['amazigh'])
            )
        else:
...
file_path_v = "verbs.csv"
def load_verb_data():
    data = []
    with open(file_path_v, encoding="utf-8-sig") as f:
        reader = csv.DictReader(f)
        for row in reader:
            cleaned_row = {k.strip(): v.strip() for k, v in row.items()}
            data.append(cleaned_row)
    return data
verb_data = load_verb_data()

class VerbsScreen(MDScreen):
    def on_pre_enter(self, *args):
        global verb_data
        if not any('arabic_fixed' in entry for entry in verb_data):
    ...
    def on_search_verb(self):
        ...
        if not search_term:
            return
        results = [entry for entry in verb_data if entry['verb'] == search_term]
        if results:
            result_text = ""
